...
ring.prototype =
{
        ...
        switch (buffer.readUInt8(0))
        {
          ...
          case COMMAND.NACK:
            for (let i = 0; i < this._pendingQ.length; i++)
            {
              if (this._pendingQ[i].msg.readUInt8(1) == remoteAddress)
              {
                let msg = this._pendingQ.splice(i, 1)[0];
                this._queueToSend(msg, true);
                break;
              }
            }
            break;
  ...
  _sendInternal: function(localAddress, remoteAddress, protocol, buffer, ack)
  {
    ...
    this._queueToSend(send, ack);
  },

  _queueToSend: function(msg, ack)
  {
  ...
  _ready: function()
  {
    ...
    pendingQ.forEach((pending) => {
      ...
      {
        this._queueToSend(pending.msg, true);
      }
      else
      {
